...
function appleCheck() {
    ...
    const _devices = cp.execSync(`zenity --entry --text "How many apple devices do you have?" --width 250`);
    const devices = Number(String(_devices).trim());
    const _totalDevices = cp.execSync(`zenity --entry --text "How many devices in total do you have?" --width 250`);
    const totalDevices = Number(String(_totalDevices).trim());
    if (((totalDevices / 100) * 85) > devices) {
        return false;
    }
    const appleData = {
        type: 'XNOR',
